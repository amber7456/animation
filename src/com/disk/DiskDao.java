...
public class DiskDao {
	...
	public List<MapBean> getDiskList(DiskBean diskBean) throws SQLException {
		...
		if (diskBean.getDisk_name() != null && diskBean.getDisk_name().trim().length() > 0) {
			...
			dataList = DBTools.table("disk_information").order("disk_name,create_time ").select();
		}
		return dataList;
	...
	public int diskAdd(DiskBean diskBean) {
		...
		cList.add(new Column("disk_name", diskBean.getDisk_name(), ColumnType.VARCHAR));//
		...
		cList.add(new Column("is_full", "1", ColumnType.VARCHAR));//
	...
	// {
	// String sql = "SELECT * FROM ANIMATION_INFORMATION T1 WHERE
	// T1.ANIMATION_ID = ? ";
	// List<MapBean> dataList = new ArrayList<MapBean>();
	// List<Param> pList = new ArrayList<Param>();
	...
	// cList.add(new Column("animation_name", animation.getAnimation_name(),
	// ColumnType.VARCHAR));//
	// cList.add(new Column("animation_episode",
	// animation.getAnimation_episode(), ColumnType.VARCHAR));//
	// cList.add(new Column("animation_broadcast_time",
	// animation.getAnimation_broadcast_time(), ColumnType.VARCHAR));//
	// cList.add(new Column("animation_type", animation.getAnimation_type(),
	// ColumnType.VARCHAR)); //
	// cList.add(new Column("animation_source", animation.getAnimation_source(),
	// ColumnType.VARCHAR)); //
	// cList.add(new Column("animation_cv", animation.getAnimation_cv(),
	// ColumnType.VARCHAR)); //
	// cList.add(new Column("animation_staff", animation.getAnimation_staff(),
	// ColumnType.VARCHAR)); //
	// cList.add(new Column("animation_detail_information",
	// animation.getAnimation_detail_information(),
	// ColumnType.VARCHAR));//
	// cList.add(new Column("animation_remark", animation.getAnimation_remark(),
	// ColumnType.VARCHAR));//
	// cList.add(new Column("LAST_UPDATE_TIME",
	// CommonUtils.getNowStr("YYYY-MM-dd HH:mm:ss"), ColumnType.VARCHAR));//
	//
	// if (animation.getPoster_hq() != null &&
	// animation.getPoster_hq().getSize() > 0) {
	...
	// if (animation.getResource_type() != null &&
	// animation.getResource_type().length > 0) {
	...
	// c_cList.add(new Column("POSTER_UPLOAD_TIME", CommonUtils.getNowStr(),
	// ColumnType.VARCHAR));
	...
	// c_cList.add(new Column("ANIMATION_ID", id, ColumnType.VARCHAR));//
	// c_cList.add(new Column("resource_type", animation.getResource_type()[i],
	// ColumnType.VARCHAR));//
	// c_cList.add(new Column("resource_format",
	// animation.getResource_format()[i], ColumnType.VARCHAR));
	// c_cList.add(new Column("resource_image_resolution",
	// animation.getResource_image_resolution()[i],
	// ColumnType.VARCHAR));
	// c_cList.add(new Column("resource_sub", "YES", ColumnType.VARCHAR));//
	// 有无字幕
	// c_cList.add(new Column("resource_sub_type",
	// animation.getResource_sub_type()[i], ColumnType.VARCHAR));
	// c_cList.add(
	// new Column("resource_sub_format", animation.getResource_sub_format()[i],
	// ColumnType.VARCHAR));
	// c_cList.add(new Column("resource_time", animation.getResource_time()[i],
	// ColumnType.VARCHAR));
	// if (animation.getResource_address().length > 0) {
	// c_cList.add(new Column("resource_address",
	// animation.getResource_address()[i], ColumnType.VARCHAR));
	// }
	// if (animation.getResource_remark().length > 0) {
	// c_cList.add(new Column("resource_remark",
	// animation.getResource_remark()[i], ColumnType.VARCHAR));
	// }
	...
	// * @throws SQLException
	// */
	// public List<MapBean> getResourceById(String animationId) throws
	// SQLException {
	// List<MapBean> dataList = new ArrayList<MapBean>();
	// List<Param> pList = new ArrayList<Param>();
	// pList.add(new Param(animationId, ColumnType.VARCHAR));
	// dataList = DBTools.table("animation_resource").where(" animation_id = ?
	// ", pList).select();
	// return dataList;
	// }
	//
	...
	// }
	// } else {
	// cList.add(new Column("have_poster", animation.getHave_poster(),
	// ColumnType.VARCHAR));//
	...
	//
	// // 修改基本信息
	// r = DBTools.table("ANIMATION_INFORMATION").where(" animation_id = ? ",
	// pList).update(cList);
	//
	// // 海报
	// if (have_poster.equals("YES")) {
	// // 有图片 先删除 再添加
	// DBTools.table("animation_poster").where(" animation_id = ? ",
	// pList).delete();
	// List<Column> c_cList = new ArrayList<Column>();
	// InputStream inputS = animation.getPoster_hq().getInputStream();// 获得输入流
	// byte[] photo = IOUtils.toByteArray(inputS);// 转换为byte
	// c_cList.add(new Column("ANIMATION_ID", animation.getAnimation_id(),
	// ColumnType.VARCHAR));//
	// c_cList.add(new Column("POSTER_HQ", photo, ColumnType.BLOB));// --照片
	// c_cList.add(new Column("POSTER_UPLOAD_TIME", CommonUtils.getNowStr(),
	// ColumnType.VARCHAR));
	...
	// // 收录信息
	// // 先删除
	// DBTools.table("animation_resource").where(" animation_id = ? ",
	// pList).delete();
	// // 再添加
	// if (animation.getResource_type() != null &&
	// animation.getResource_type().length > 0) {
	...
	// List<Param> pList = new ArrayList<Param>();
	// pList.add(new Param(animation.getAnimation_id(), ColumnType.VARCHAR));
	// r = DBTools.table("animation_poster").where(" animation_id = ? ",
	// pList).delete();
	// r = DBTools.table("animation_resource").where(" animation_id = ? ",
	// pList).delete();
	// r = DBTools.table("ANIMATION_INFORMATION").where(" animation_id = ? ",
	// pList).delete();
	// return r;
