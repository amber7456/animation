...
public class Test {
	public static void main(String[] args) throws Exception {
		ArrayList<String[]> list03 = readExcel("D:/excel/0302.xlsx");

		ArrayList<String> list = getAnimation();
		for (int i = 0; i < list03.size(); i++) {

			boolean flag = true;
			for (int j = 0; j < list.size(); j++) {
				if (list03.get(i)[1].trim().equals(list.get(j).trim())) {
					flag = false;
					break;
				}
			}
			if (flag) {
				System.out.println(list03.get(i)[0] + "\t" + list03.get(i)[1]); 
			}
		}
	}
